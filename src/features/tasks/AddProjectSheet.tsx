...
const ICONS = ['briefcase.fill', 'house.fill', 'book.fill', 'heart.fill', 'airplane', 'graduationcap.fill', 'gamecontroller.fill', 'flag.fill'];

export function AddProjectSheet({ visible, onClose }: { visible: boolean; onClose: () => void }) {
  const { colors, typography, spacing } = useTheme();
  const { projects, addProject } = useProductivityStore();
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await addProject({ name: name.trim(), icon, color, sort_order: projects.length });
      setName('');
      onClose();
    } finally {
      setSaving(false);
  ...
  return (
    <Sheet visible={visible} onClose={onClose}>
      <ScrollView contentContainerStyle={{ paddingHorizontal: spacing.lg }} keyboardShouldPersistTaps="handled">
        <Text style={[typography.title2, { color: colors.label, marginBottom: spacing.md }]}>New Project</Text>
        <TextField label="Name" placeholder="e.g. Wedding, Move house" value={name} onChangeText={setName} autoFocus />
