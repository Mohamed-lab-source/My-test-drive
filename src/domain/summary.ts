...
export function buildProgressSummary(habits: Habit[], checkins: CheckIn[]): string {
...
export interface YearInReview {
  year: number;
  totalVotes: number;
  bestMonth: { name: string; votes: number } | null;
  bestStreakEver: number;
  mostImproved: { habit: Habit; before: number; after: number } | null;
}
...
export function computeYearInReview(habits: Habit[], checkins: CheckIn[]): YearInReview {
  ...
  const monthVotes = new Map<number, number>();
  ...
  let bestMonth: YearInReview["bestMonth"] = null;
  for (const [index, votes] of monthVotes) {
    if (!bestMonth || votes > bestMonth.votes) bestMonth = { name: MONTH_NAMES[index]!, votes };
  }
  ...
  let mostImproved: YearInReview["mostImproved"] = null;
  if (h1Dates.length > 0 && h2Dates.length > 0) {
    for (const h of active) {
      ...
      if (h1Due < MIN_DUE_FOR_COMPARISON || h2Due < MIN_DUE_FOR_COMPARISON) continue;
...
export function buildYearInReviewSummary(habits: Habit[], checkins: CheckIn[]): string {
  const r = computeYearInReview(habits, checkins);
  const lines = [`My ${r.year} in Atomic Habits:`, `🗳️ ${r.totalVotes} votes cast this year`];
  if (r.bestMonth) lines.push(`📅 Best month: ${r.bestMonth.name} (${r.bestMonth.votes} votes)`);
  lines.push(`🔥 ${r.bestStreakEver}-day longest streak`);
  if (r.mostImproved) {
    lines.push(
      `📈 Most improved: ${r.mostImproved.habit.name} (${Math.round(r.mostImproved.before * 100)}% → ${Math.round(r.mostImproved.after * 100)}%)`
    );
  }
