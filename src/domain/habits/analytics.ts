...
import { addDays, isDue, todayISO, weekdayOf } from './dateUtils';

export function findCheckIn(checkins: CheckIn[], habitId: string, date: string): CheckIn | undefined {
...
export function identityVoteSeries(
  ...
  const habitIds = new Set(habits.filter((h) => h.identityId === identityId).map((h) => h.id));
  return dates.map(
    (date) => checkins.filter((c) => habitIds.has(c.habitId) && c.date === date && isVote(c)).length
  );
}

/** Fraction (0..1) of due occurrences that were completed, over the given dates. */
export function completionRate(habit: Habit, checkins: CheckIn[], dates: string[]): number {
...
export function dailyConsistency(
  ...
  return dates.map((date) => {
    ...
    const due = active.filter((h) => isDue(h.frequency, date)).filter((h) => !isNeutralized(findCheckIn(checkins, h.id, date)));
