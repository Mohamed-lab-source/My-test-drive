...
import { isVote } from './analytics';

const XP_FULL = 10;
const XP_TWO_MINUTE = 5;

export function computeXP(checkins: CheckIn[]): number {
  return checkins.reduce((sum, c) => {
    if (c.completedFull) return sum + XP_FULL;
    if (c.usedTwoMinuteVersion) return sum + XP_TWO_MINUTE;
    return sum;
  }, 0);
}
...
export function levelForXP(xp: number): LevelInfo {
  let level = 1;
  while (xpRequiredForLevel(level) <= xp) level++;
...
export function freezeTokenBalance(checkins: CheckIn[]): number {
