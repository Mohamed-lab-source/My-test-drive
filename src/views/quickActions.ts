...
export function openQuickActions(title: string, actions: QuickAction[]): void {
    ...
    <div class="modal-sheet quick-actions-sheet" role="dialog" aria-modal="true">
      ...
      <div class="quick-actions-list">
        ${actions
          .map(
            (a, i) =>
              `<button type="button" class="quick-action-btn ${a.danger ? "quick-action-danger" : ""}" data-action-index="${i}">${escapeHtml(a.label)}</button>`
          )
          .join("")}
      </div>
  ...
  container.querySelectorAll<HTMLButtonElement>("[data-action-index]").forEach((btn) => {
