...
function search(query) {
    ...
    if (!q)
        return { habits: [], identities: [], scorecard: [] };
    return {
...
function buildResultsHtml(query, results) {
    const total = results.habits.length + results.identities.length + results.scorecard.length;
    if (!query.trim())
        return `<div class="empty-state"><p>Start typing to search habits, identities, and your scorecard.</p></div>`;
    if (total === 0) {
...
export function openSearch() {
    ...
    function wireResults() {
        ...
        resultsEl.querySelectorAll("[data-open-habit]").forEach((btn) => {
        ...
        resultsEl.querySelectorAll("[data-open-identity]").forEach((btn) => {
            btn.addEventListener("click", () => {
        ...
        resultsEl.querySelectorAll("[data-open-scorecard]").forEach((btn) => {
            btn.addEventListener("click", () => {
                close();
