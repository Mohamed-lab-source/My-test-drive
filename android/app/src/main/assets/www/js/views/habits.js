...
function renderChainTree(habit, visibleHabits, identityLabel, depth, index, seen, rootInfo) {
    seen.add(habit.id);
...
function buildListHtml(activeHabits, identityLabel) {
    ...
    const visible = tagFilter.size === 0 ? filtered : filtered.filter((h) => h.tags.some((t) => tagFilter.has(t)));
    ...
    roots.forEach((h, i) => {
        if (seen.has(h.id))
            return;
        const rootInfo = canReorder ? { isFirst: i === 0, isLast: i === roots.length - 1 } : null;
...
export function renderHabits(container) {
    ...
    <section class="view">
      <header class="view-header view-header-with-action">
        <div>
          <h1>Habits</h1>
          <p class="view-subtitle">${selectMode
        ? `${selectedIds.size} selected`
        : "Design each habit with the Four Laws, scale it down with the 2-minute rule, and stack it onto something you already do."}</p>
        </div>
      ...
      ${allTags.length > 0 && !selectMode
        ? `<div class="tag-filter-bar">
              ${allTags
            .map((t) => `<button type="button" class="tag-chip ${tagFilter.has(t) ? "active" : ""}" data-tag-filter="${escapeHtml(t)}">${escapeHtml(t)}</button>`)
            .join("")}
            </div>`
        : ""}
...
function wireListInteractions(listEl, habits, container) {
    ...
    listEl.querySelectorAll("[data-select-habit]").forEach((cb) => {
