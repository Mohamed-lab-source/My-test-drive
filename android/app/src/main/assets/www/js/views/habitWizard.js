 ...
 */
function wouldCreateCycle(anchorId, editingId, habits) {
    if (!editingId)
        return false;
    let cursor = anchorId;
    const seen = new Set();
...
export function openHabitWizard(options = {}) {
    ...
    function updateStep(animateForward = true) {
    ...
    function renderStep(name, body) {
        if (name === "template")
            return renderTemplateStep(body);
        if (name === "name")
            return renderNameStep(body);
        if (name === "when")
            return renderWhenStep(body);
        if (name === "stick")
            return renderStickStep(body);
        renderReviewStep(body);
    }
    // ---- Step: template picker ----
    function renderTemplateStep(body) {
        body.innerHTML = `
    ...
    `;
        const nameInput = body.querySelector("#wizard-name");
        const nextBtn = body.querySelector("#wizard-next");
      ...
      <div class="form-card">
        ...
        <div id="wizard-weekday-picker" class="form-card-row weekday-picker ${state.freqType === "weekdays" ? "" : "hidden"}">
          ${WEEKDAY_LABELS.map((label, i) => `
            <input type="checkbox" id="w-weekday-${i}" class="chip-input" data-weekday="${i}" ${state.weekdays.has(i) ? "checked" : ""} />
    ...
    `;
        body.querySelector("#wizard-identity").addEventListener("change", (e) => {
            state.identityId = e.target.value;
        });
        const weekdayPicker = body.querySelector("#wizard-weekday-picker");
        body.querySelectorAll('input[name="w-freq"]').forEach((radio) => {
        ...
        const stackEmptyNote = body.querySelector("#wizard-stack-empty");
        body.querySelectorAll('input[name="w-stack"]').forEach((radio) => {
            radio.addEventListener("change", () => {
                state.stackType = radio.value;
    ...
    function renderReviewStep(body) {
        ...
        const anchorLabel = anchor.type === "habit"
            ...
            : anchor.type === "custom"
                ? `After ${anchor.text}`
                : null;
        const freqLabel = state.freqType === "daily"
            ? "Every day"
    ...
    `;
        ...
        body.querySelector("#wizard-back")?.addEventListener("click", () => goTo(step - 1));
    }
    // ---- Shared nav / thumb helpers ----
    function wizardNav(opts) {
        return `
    ...
    function wireNav(body, canAdvance) {
        body.querySelector("#wizard-back")?.addEventListener("click", () => goTo(step - 1));
        const nextBtn = body.querySelector("#wizard-next");
...
//# sourceMappingURL=habitWizard.js.map