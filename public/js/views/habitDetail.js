 ...
 */
function weekdayTip(habit, checkins) {
    ...
    if (best.weekday === worst.weekday || best.rate - worst.rate < 0.3)
        return null;
...
function findMostRecentMiss(habit, checkins, today) {
    ...
    for (let i = 0; i < 84; i++) {
        ...
        if (isDue(habit.frequency, cursor)) {
            ...
            if (!isVote(checkin) && !isNeutralized(checkin))
                return cursor;
        }
