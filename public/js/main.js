...
import { currentRoute, onRouteChange, consumeShortcutParam } from "./router.js";
...
function renderCrashFallback(appEl, message) {
    appEl.innerHTML = `
...
async function main() {
    ...
    try {
        ...
        if (appEl)
            renderCrashFallback(appEl, "Your habit data couldn't be loaded.");
        return;
    }
    const start = () => {
        render();
