...
export function computeCorrelationInsights(habits, checkins, limit = 3) {
    ...
    for (const a of active) {
        for (const b of active) {
            ...
            if (b.stackAnchor.type === "habit" && b.stackAnchor.habitId === a.id)
                continue;
            ...
            if (bothDueDates.length < MIN_SAMPLE)
                continue;
            ...
            if (aDoneDates.length < MIN_SUBSET_SAMPLE || aNotDoneDates.length < MIN_SUBSET_SAMPLE)
                continue;
